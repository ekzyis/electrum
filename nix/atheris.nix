...
buildPythonPackage (finalAttrs: {
  ...
  # Atheris hooks deeply into CPython internals, so it only works with the
  # interpreter versions upstream explicitly supports.
  disabled = pythonOlder "3.11" || pythonAtLeast "3.15";

  src = fetchFromGitHub {
    owner = "google";
    repo = "atheris";
    # 3.1.0 was released to PyPI as wheels only, without a git tag; this is the
    # commit that bumped the version and added CPython 3.14 support.
    rev = "352d5f29d811e0b51807ca57ed5551dd08ece528";
    hash = "sha256-LT6pjTNOhXeL76w2HTiHsBy9cv3W5CGwGPP/qKskJBU=";
  };

  # setup.py shells out to these helper scripts, whose `#!/bin/bash` shebangs
  # are not valid inside the build sandbox.
  postPatch = ''
  ...
  # Point Atheris at the libFuzzer archive shipped with LLVM's compiler-rt
  # instead of letting it discover one via `clang -print-search-dirs`.
  ...
  passthru.tests = {
    # Smoke test that exercises the full fuzzing pipeline: instrument a target,
    # let libFuzzer's coverage feedback drive it to a planted crash, and assert
    # the Python exception is surfaced. This catches breakage in the native
    # extensions and the libFuzzer linkage that a plain import check would miss.
    smoke =
